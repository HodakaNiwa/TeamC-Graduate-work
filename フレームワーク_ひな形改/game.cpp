...
void CGame::Init(void)
{
	...
	{
		m_pCharacter[nCntEnemy] = CEnemy::Create(nCntEnemy, CEnemy::TYPE_BRAZIL, D3DXVECTOR3(0.0f, 0.0f, 0.0f),
			"data\\TEXT\\ModelLoad\\ENEMY_MODEL.txt", "data\\TEXT\\MotionLoad\\ENEMY_MOTION.txt");
		m_nEnemyNum++;
	}
	...
	{//プレイ人数分回す
		...
		{//スピード型プレイヤーの生成
			m_pCharacter[nCnt] = CSpeedPlayer::Create(D3DXVECTOR3(-50.0f + (nCnt * 50.0f), 0.0f, 0.0f),
				...
				"data\\TEXT\\MotionLoad\\SPEEDPLAYER_MOTION.txt", nCnt, m_nCountry[nCnt], CCharacter::CHARCTERTYPE_SPEED);	//体下
		}
		...
		{//パワー型プレイヤーの生成
			m_pCharacter[nCnt] = CPowerPlayer::Create(D3DXVECTOR3(-50.0f + (nCnt * 50.0f), 0.0f, 0.0f),
				...
				"data\\TEXT\\MotionLoad\\POWERPLAYER_MOTION.txt", nCnt, m_nCountry[nCnt], CCharacter::CHARCTERTYPE_POWER);
		}
...
void CGame::ReleaseUI(void)
{
	if (m_pUI != NULL)
	{
		...
		for (int nCnt = 0; nCnt < MAX_CHARACTER; nCnt++)
		{
			m_nScore[nCnt] = m_pUI->GetScoreGame(nCnt)->GetScore();
		}
