...
public:
	...
	//	---<<基盤関数>>---
	HRESULT Init(int nCharaNum, char ModelTxt[40], char MotionTxt[40]);
	void Uninit(void);
	void Update(void);
	void Draw(void);
	void Unload(void);
	...
	int GetCuntryResult(int nNumCuntry) { return m_nResultCuntry[nNumCuntry]; }
...
protected:
	...
	static CHARCTERTYPE m_CharcterTypeResult[4];		//リザルト用のキャラタイプ
	static  int m_nResultCuntry[4];						//リザルト用の国タイプ
private:
