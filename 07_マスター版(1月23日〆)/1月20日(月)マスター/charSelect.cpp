...
HRESULT CSelectCharacter::Init(D3DXVECTOR3 pos, D3DXVECTOR3 size, int nCuntry, int nNumPlayer, int nNumControler)
{
	...
	m_InputType = CSelect::INPUTTYPE_MAX;	//入力デバイスのd設定
...
void CSelectCharacter::UpdateInput(void)
{
	...
	if (m_InputType == CSelect::INPUTTYPE_GAMEPAD)
	{
		...
		{//右移動
			pSound->PlaySound(CSound::SOUND_LABEL_SE009);	 //セレクト音
			MoveRight();
		}
		...
		{
			pSound->PlaySound(CSound::SOUND_LABEL_SE007);	 //決定音
			PressEnter();
		}
	}
	else if (m_InputType == CSelect::INPUTTYPE_MOUSE)
	{
		...
		{
			pSound->PlaySound(CSound::SOUND_LABEL_SE007);	 //セレクト音
			PressEnter();
		}
	}
	else
...
void CSelectCharacter::LoadTex(void)
{
	...
	//ステータステクスチャの読み込み
	D3DXCreateTextureFromFile(pDevice, TEX_STATUS, &m_pStatusTex);	//キャラ決定
}
