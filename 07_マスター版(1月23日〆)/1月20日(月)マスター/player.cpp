...
void CPlayer::PlayerMoveKeyboard(D3DXVECTOR3 &CameraRot, D3DXVECTOR3 &pos)
{
	...
	if (m_PlayerState != PLAYERSTATE_BLOWAWAY && m_PlayerState != PLAYERSTATE_DAMAGE  && m_PlayerState != PLAYERSTATE_ROBOTDAMAGE)
	{
		...
		else
		{
			...
			{
				pSound->StopSound(CSound::SOUND_LABEL_SE018);	//SEを止める
				m_PlayerState = PLAYERSTATE_NONE;				//ニュートラル状態にする
				m_pMotion->SetNumMotion(PLAYERSTATE_NONE);		//待機モーションにする
			}
		}
	}
	...
	{//スプリントモーション
		...
		{
			pSound->PlaySound(CSound::SOUND_LABEL_SE018);	//足音
			m_pMotion->SetNumMotion(PLAYERSTATE_ACTION);	//アクションモーションにする
			m_bSprintMotion = false;						//モーションをループしないようにする
		}
	}
		...
		{// 地雷だったら
			...
			{// 自分以外が出した地雷ならダメージf
				...
				if (m_nDamageCounter == 0)
				{
					...
					m_nDamageCounter = 1;							//ダメージを一回だけ通るようにする
					if (m_nDownCount >= 1)
					{//ダウンカウントが1以上だった場合初期化する
						m_nDownCount = 0;
					}
				}
			}
