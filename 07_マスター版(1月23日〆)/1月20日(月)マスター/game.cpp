...
#define DIVISIONEVENT_COLLIDER_MIN			(15.0f)
...
//タイマーのサイズ 
#define TIMER_SIZE		(D3DXVECTOR3(100.0f, 130.0f, 0.0f))	//全体のタイマー
#define TIMER_SIZE_4	(D3DXVECTOR3(75.0f, 99.0f, 0.0f))	//４人用のタイマー

// ロボット
#define ROBOTEVENT_TIME						(7380)		// 出現時間7380
#define ROBOTEVENT_TIMEDEBUG				(100)		// 出現時間
...
CGame::CGame()
{
	...
	for (int nCnt = 0; nCnt < MAX_CHARACTER; nCnt++)
	{
		...
		InitPos[nCnt] = INITIALIZE_VECTOR3;
	}
...
void CGame::Init(void)
{
	...
	CExprosionTrace::LoadTex();			//爆発跡
	CCrack::LoadTex();					//ひび割れ
	CRipples::LoadTex();				//波紋
	CNumberBillborad::LoadTex();		//ナンバー
	CPoint::LoadTex();					//プラス
	CBullet::LoadModel();				// ロボットの弾
	CShadow::LoadTex();					//影　
	CScoreGame::LoadTex();				//スコアフレーム
...
void CGame::CreateRobot(void)
{
	...
	m_pRoboCharacter->CreateRobotUI();
}
...
void CGame::CreateInitCamera(void)
{
	...
	{//プレイ人数が1人だったら
	 ...
	 //カウントダウンタイマーの生成
		m_pTime[0] = CTimer::Create(CTimer::TYPE_COUNTER, 7, D3DXVECTOR3(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, 0.0f), TIMER_SIZE);

		//ゲームタイマーの生成（ラスト5秒で描画される
		m_pTime[1] = CTimer::Create(CTimer::TYPE_GAME, 7, D3DXVECTOR3(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, 0.0f), TIMER_SIZE);
		...
		{//カメラの生成(1P:左上)
			...
			m_pGameCamera[0]->Init(m_pCharacter[0]);
	...
	{//プレイ人数が2人だったら
		...
		//カウントダウンタイマーの生成
		m_pTime[0] = CTimer::Create(CTimer::TYPE_COUNTER, 7, D3DXVECTOR3(SCREEN_WIDTH / 4, SCREEN_HEIGHT / 2, 0.0f), TIMER_SIZE);
		m_pTime[1] = CTimer::Create(CTimer::TYPE_GAME, 7, D3DXVECTOR3(SCREEN_WIDTH / 4, SCREEN_HEIGHT / 2, 0.0f), TIMER_SIZE);

		//カウントダウンタイマーの生成
		CTimer::Create(CTimer::TYPE_COUNTER, 7, D3DXVECTOR3((SCREEN_WIDTH / 4) * 3, SCREEN_HEIGHT / 2, 0.0f), TIMER_SIZE);
		CTimer::Create(CTimer::TYPE_GAME, 7, D3DXVECTOR3((SCREEN_WIDTH / 4) * 3, SCREEN_HEIGHT / 2, 0.0f), TIMER_SIZE);
		...
		{//カメラの生成(2P:右上)
			...
			m_pGameCamera[1]->Init(m_pCharacter[1]);
	...
	{//プレイ人数が3人だったら
	 ...
	 //カウントダウンタイマーの生成
		m_pTime[0] = CTimer::Create(CTimer::TYPE_COUNTER, 7, D3DXVECTOR3(SCREEN_WIDTH / 4, SCREEN_HEIGHT / 4, 0.0f), TIMER_SIZE_4);
		m_pTime[1] = CTimer::Create(CTimer::TYPE_GAME, 7, D3DXVECTOR3(SCREEN_WIDTH / 4, SCREEN_HEIGHT / 4, 0.0f), TIMER_SIZE_4);

		//カウントダウンタイマーの生成
		CTimer::Create(CTimer::TYPE_COUNTER, 7, D3DXVECTOR3((SCREEN_WIDTH / 4) * 3, SCREEN_HEIGHT / 4, 0.0f), TIMER_SIZE_4);
		CTimer::Create(CTimer::TYPE_GAME, 7, D3DXVECTOR3((SCREEN_WIDTH / 4) * 3, SCREEN_HEIGHT / 4, 0.0f), TIMER_SIZE_4);

		//カウントダウンタイマーの生成
		CTimer::Create(CTimer::TYPE_COUNTER, 7, D3DXVECTOR3(SCREEN_WIDTH / 4, (SCREEN_HEIGHT / 4) * 3, 0.0f), TIMER_SIZE_4);
		CTimer::Create(CTimer::TYPE_GAME, 7, D3DXVECTOR3(SCREEN_WIDTH / 4, (SCREEN_HEIGHT / 4) * 3, 0.0f), TIMER_SIZE_4);
		...
		{//カメラの生成(3P:左下)
			...
			m_pGameCamera[2]->Init(m_pCharacter[2]);
	...
	{//プレイ人数が4人だったら
	 ...
	 //カウントダウンタイマーの生成
		...
		//カウントダウンタイマーの生成
		CTimer::Create(CTimer::TYPE_COUNTER, 7, D3DXVECTOR3((SCREEN_WIDTH / 4) * 3, (SCREEN_HEIGHT / 4) * 3, 0.0f), TIMER_SIZE_4);
		CTimer::Create(CTimer::TYPE_GAME, 7, D3DXVECTOR3((SCREEN_WIDTH / 4) * 3, (SCREEN_HEIGHT / 4) * 3, 0.0f), TIMER_SIZE_4);
		...
		{//カメラの生成(4P:右下)
			...
			m_pGameCamera[3]->Init(m_pCharacter[3]);
...
{	
	...
	CCrack::UnloadTex();		  
	CRipples::UnloadTex();	
	CNumberBillborad::UnloadTex();
	CPoint::UnloadTex();
	CBullet::UnloadModel();
	CShadow::UnloadTex();		
	CScoreGame::UnloadTex();			
...
void CGame::Update(void)
{
	...
	// ロボットイベントの生成
#ifdef ROBOT_EVENT
	if (m_pRoboCharacter == NULL && m_gameState == GAMESTATE_NORMAL && m_bCreateRoboto == false)		// ←追加(よしろう)
	{
		if (m_nCntRobot == ROBOTEVENT_TIME)
		{
			pSound->PlaySound(CSound::SOUND_LABEL_SE034);	
			pSound->SetVolume(CSound::SOUND_LABEL_SE034, 3.0f);
			CreateRobot();				// ロボットの生成
			CreateInitEventCamera();	// イベントカメラの生成
			m_bCreateRoboto = true;
			m_nCntRobot = 0;
		}
		else
		{
...
void CGame::SprintFlag(void)
{
	...
	for (int nCnt = 0; nCnt < m_nNumPlay; nCnt++)
	{
		if (m_pCharacter[nCnt] != NULL)
		{
			...
			bool bWalk = m_pCharacter[nCnt]->GetWalkFrag();
