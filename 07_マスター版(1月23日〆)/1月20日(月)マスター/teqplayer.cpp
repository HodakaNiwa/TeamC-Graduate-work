...
void  CTechniquePlayer::ActionUpdate(void)
{
	...
	{//マインを設置中
		...
		if (m_nInstallationCounter % 30 == 0)
		{
			...
			{
				if (m_PlayerState != PLAYERSTATE_DAMAGE && m_PlayerState != PLAYERSTATE_BLOWAWAY && m_PlayerState != PLAYERSTATE_ROBOTDAMAGE)
				{
					//地雷を設置
...
void  CTechniquePlayer::PlayerActionPad(void)
{
	...
	if (pXPad->GetTrigger(XINPUT_GAMEPAD_B, m_nControllerIndx) == true)
	{
		if (m_PlayerState != PLAYERSTATE_DAMAGE && m_PlayerState != PLAYERSTATE_ACTION
			&& m_PlayerState != PLAYERSTATE_BLOWAWAY && m_PlayerState != PLAYERSTATE_ROBOTDAMAGE)
		{
			//マインポイントが20以上なら設置可能
	...
	CHARCTERTYPE charctertype, int nControllerIndx, int nControllerType)
{
	...
	if (pTeqPlayer == NULL)
	{
		...
		{
			MessageBox(0, "地雷プレイヤーがNULLでした", "警告", MB_OK);
		}
	}
	else
	{
		MessageBox(0, "地雷プレイヤーがNULLじゃないです", "警告", MB_OK);
	}
	return pTeqPlayer;
}
