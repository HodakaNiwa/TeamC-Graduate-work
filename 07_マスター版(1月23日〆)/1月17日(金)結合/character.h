...
public:
	...
	void SetGetScorePoint(int nGetScorePoint) { m_nGetScorePoint = nGetScorePoint; }
	void SetDestPlayer(int nDestPlayer) { m_nDestPlayer = nDestPlayer; }
	D3DXVECTOR3 GetPos(void) { return m_Pos; }
	D3DXVECTOR3 *GetPosPointer(void) { return &m_Pos; }
	D3DXVECTOR3 GetRot(void) { return m_Rot; }
	int GetGetScorePoint(void) { return m_nGetScorePoint; }
	int GetDestPlayer(void) { return m_nDestPlayer; }
	...
	CTerritory *GetTerritoryTop(void) { return m_apTerritory[0]; }
	...
	//スプリント中どうか
	bool GetSprint(void) { return m_bSprint; }
	void SetSprint(bool bSprint) { m_bSprint = bSprint; }

	//歩いている最中か
	bool GetWalkFrag(void) { return m_bWalkFrag; }
	void SetWalkFrag(bool bWalkFrag) { m_bWalkFrag = bWalkFrag; }
	
protected:
	//	---<<コリジョン関連関数>>---
	virtual void CreateCylinderCollider(void);

	void CountDownLineTime(void);												//テリトリーの取得更新処理
	bool ChackSamePos(CTerritory * pTerritory, int nNumTerritory);				//同じ位置かどうか
	void ShapeComplete(CTerritory * pTerritory, int * nGetPoint);				//完成したときの処理
	void ChackInShape(int * nGetPoint);											//図形に入っているかどうかの処理
	...
	void Interruption(void);													//テリトリー取得を中断

	void MakeTraiangle(int nCnt, int * nCountTraiangle, int * pnFarNumTerritory, D3DXVECTOR3 * pListPos, TRAIANGLE * pTraiangle);	
	//三角形を作る処理
...
private:
	...
	void CreateLightOrbit(CGame *pGame);										// 光の軌跡を作る処理
	void AddTerritoryPoint(CTerritory * pTerritory, int nOldPlayer);			// テリトリースコアの追加
