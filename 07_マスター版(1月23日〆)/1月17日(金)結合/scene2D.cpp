...
CScene2D * CScene2D::Create(const D3DXVECTOR3 pos, D3DXVECTOR3 size, TYPE type)
{
	...
	{
		pScene2D = new CScene2D(6, CScene::OBJTYPE_2DPOLYGON);
	}
...
CScene2D::CScene2D() : CScene(6, CScene::OBJTYPE_2DPOLYGON)
{
	...
	m_Size = VECTOR_ZERO;				// ポリゴンのサイズ
	m_bUse = false;						// 使用しているかどうか
	m_Tex = D3DXVECTOR2(0.0f, 1.0f);	// UV値
	m_Color = INITIALIZE_VECTOR4;		// 色
	m_fHight = 0.0f;					// 辺の高さ
	m_fHight2 = 0.0f;					// 辺の高さ
}
...
// 回転ポリゴンの座標設定処理
//=============================================================================
void CScene2D::SetRotPos(float fAngle)
{
	VERTEX_2D*pVtx;			//頂点情報へのポインタ
...
// 頂点ごとのサイズ設定
//=============================================================================
void CScene2D::SetSizeVertex(float fHight, float fHight2)
{
	//高さの値を保存
	m_fHight = fHight;
	m_fHight2 = fHight2;
	...
	// 頂点情報を設定
	pVtx[0].pos = D3DXVECTOR3(m_pos.x - m_Other.x, m_pos.y - (m_fHight * 0.5f), 0.0f);
	pVtx[1].pos = D3DXVECTOR3(m_pos.x + m_Other.x, m_pos.y - (m_fHight2 * 0.5f), 0.0f);
	pVtx[2].pos = D3DXVECTOR3(m_pos.x - m_Other.x, m_pos.y + (m_fHight * 0.5f), 0.0f);
	pVtx[3].pos = D3DXVECTOR3(m_pos.x + m_Other.x, m_pos.y + (m_fHight2 * 0.5f), 0.0f);
...
// 頂点ごとの位置設定
//=============================================================================
void CScene2D::SetPosVertex(D3DXVECTOR3 Pos)
{
	//位置設定
	m_pos = Pos;
...
// 画像を左右反転させる
//=============================================================================
void CScene2D::SetReflectRightLeft(void)
{
	VERTEX_2D*pVtx;		//頂点情報へのポインタ
