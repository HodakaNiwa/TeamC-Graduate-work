...
//			Author : Kobayashi_Sho-hei / 小林 将兵 & Hodaka Niwa & 佐藤　安純
//	◆　◆　◆　◆　◆　◆　◆　◆　◆　◆　◆　◆　◆　◆　◆　◆　◆　◆
...
#define SHADOW_SIZE					(D3DXVECTOR3(50.0f, 0.0f, 50.0f))
...
HRESULT CCharacter::Init(int nCharaNum, char ModelTxt[40], char MotionTxt[40],int nType, int nCuntry)
{
	...
	if (m_pShadow == NULL)
	{
		m_pShadow = CShadow::Create(m_Pos, SHADOW_SIZE);
	}
	return S_OK;
}
...
void CCharacter::Update()
{
	...
	//影の更新処理 安
	if (m_pShadow != NULL) { m_pShadow->SetPos(D3DXVECTOR3(m_Pos.x, 0.08f, m_Pos.z)); }
