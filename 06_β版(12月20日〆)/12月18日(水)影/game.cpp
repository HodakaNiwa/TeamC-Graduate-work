...
// ロボット
#define ROBOTEVENT_TIME						(7380)		// 出現時間
...
void CGame::CreateUI(void)
{
	...
	for (int nCnt = 0; nCnt < m_nNumPlay; nCnt++)
	{
		...
		if (m_nPlayerType[nCnt] == 0)
		{//スピード型プレイヤーの生成
			m_pUI->CreateSkilicon(m_pCharacter[nCnt], nCnt, 21 * 60);	
		}
		else if (m_nPlayerType[nCnt] == 1)
		{//パワー型プレイヤーの生成
...
void CGame::Update(void)
{
	...
	{
		if (m_nCntRobot == ROBOTEVENT_TIMEDEBUG)
		{
			CreateRobot();				// ロボットの生成
	...
	if (!m_bDebugCamera)
	{
		if (m_nNumPlay == 1)
		{
			...
			CManager::GetRenderer()->GetDevice()->Clear(0,
				...
				D3DCOLOR_RGBA(255, 100, 100, 255), 1.0f, 0);
			//すべて描画する(左上)
			CScene::DrawAll();
