...
void CSelect::Update(void)
{
	...
	case STATE_BACK_CHRSELECT: UpdateBackCharSelect(); break;		//キャラ選択に戻る
...
void CSelect::UpdateCuntrySelect(void)
{
	...
	//キーボード入力
	if (m_nEntryPlayer < NUMPLAYER)
	{
		if (pKeyboard->GetKeyboardTrigger(DIK_RETURN) == true)
		{
			PlayerEnter(m_nEntryPlayer, INPUTTYPE_KEYBOARD);
			//m_nNumControler = m_nEntryPlayer;
		}
	}
	...
	if (pKeyboard->GetKeyboardTrigger(DIK_2) == true)
	{
		if (m_nEntryPlayer <= 1)
		{
			PlayerEnter(1, INPUTTYPE_KEYBOARD);
		}

		m_nNumControler = 1;
	}
	if (pKeyboard->GetKeyboardTrigger(DIK_3) == true)
	{
		if (m_nEntryPlayer <= 2)
		{
			PlayerEnter(2, INPUTTYPE_KEYBOARD);
		}

		m_nNumControler = 2;
	}
	if (pKeyboard->GetKeyboardTrigger(DIK_4) == true)
	{
		if (m_nEntryPlayer <= 3)
		{
			PlayerEnter(3, INPUTTYPE_KEYBOARD);
		}
...
void CSelect::UpdateCharacterSelect(void)
{
	...
	{//安
		if (m_anInputType[nCntEntry] == INPUTTYPE_GAMEPAD)
		{
			if (pXPad->GetTrigger(XINPUT_GAMEPAD_START, m_anControler[nCntEntry]) == true)
			{
				pSound->PlaySoundA(CSound::SOUND_LABEL_SE008);
				SetState(STATE_BACKCHANGE);
			}
		}
		else if (m_anInputType[nCntEntry] == INPUTTYPE_MOUSE)
		{
			//マウスの入力処理
			if (pRawMouse->GetPress(m_anControler[nCntEntry], CRawMouse::RIMS_BUTTON_LEFT) == true)
			{
				pSound->PlaySoundA(CSound::SOUND_LABEL_SE008);
				SetState(STATE_BACKCHANGE);
			}
	...
	//決定数が同じになったら決定状態にする
	if (nCntEnter == m_nEntryPlayer)
	{ 
		pSound->SetVolume(CSound::SOUND_LABEL_SE030, 0.5f);
		pSound->PlaySoundA(CSound::SOUND_LABEL_SE031);
		pSound->PlaySoundA(CSound::SOUND_LABEL_SE032);	
		SetState(STATE_DICISION); 
	}
}
...
void CSelect::UpdateDicision(void)
{
	...
	else
	{
		...
		if (Pos.x > 680.0f)
		{
			...
			for (int nCnt = 0; nCnt < NUMPLAYER; nCnt++)
			{//安
				if (m_anInputType[nCnt] == INPUTTYPE_GAMEPAD)
				{
					if (pXPad->GetTrigger(XINPUT_GAMEPAD_B, m_anControler[nCnt]) == true)
					{
						SetPlayerInfo();	//プレイヤー情報の保存
						m_bFadeInGame = true;
					}
					if (pXPad->GetTrigger(XINPUT_GAMEPAD_START, m_anControler[nCnt]) == true)
					{
						pSound->StopSound(CSound::SOUND_LABEL_SE031);
						pSound->PlaySoundA(CSound::SOUND_LABEL_SE008);
						SetState(STATE_BACK_CHRSELECT);
					}
				}
				else if (m_anInputType[nCnt] == INPUTTYPE_MOUSE)
				{
					if (pRawMouse->GetTrigger(m_anControler[nCnt], CRawMouse::RIMS_BUTTON_RIGHT) == true)
					{
						SetPlayerInfo();	//プレイヤー情報の保存
						m_bFadeInGame = true;
					}
					if (pRawMouse->GetTrigger(m_anControler[nCnt], CRawMouse::RIMS_BUTTON_LEFT) == true)
					{
						pSound->StopSound(CSound::SOUND_LABEL_SE031);
						pSound->PlaySoundA(CSound::SOUND_LABEL_SE008);
						SetState(STATE_BACK_CHRSELECT);
...
// キャラクター選択に戻る処理
//=============================================================================
void CSelect::UpdateBackCharSelect(void)
{
	if (m_pRedyFight != NULL)
	{
		m_pRedyFight->UpdateMoveDest(D3DXVECTOR3(-640.0f, 340.0f, 0.0f), 60.0f);

		if (m_pRedyFight->GetPos().x == -640.0f)
		{
			for (int nCnt = 0; nCnt < NUMPLAYER; nCnt++)
			{//決定状態じゃなくする
				if (m_pSelectChar[nCnt] != NULL)
				{
					m_pSelectChar[nCnt]->SetEnter(false);
				}
			}
...
// フェードアウトの更新処理
//=============================================================================
void CSelect::UpdateFadeOut(void)
{
	if (m_bFadeInGame)
	{
