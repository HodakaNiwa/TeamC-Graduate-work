...
{
	CResultCharUI *pSelectChar = NULL;

	if (pSelectChar == NULL)
	{
		pSelectChar = new CResultCharUI;
...
HRESULT CResultCharUI::Init(D3DXVECTOR3 pos, D3DXVECTOR3 size, int nNumPlayer, int nRank, int nCuntry, int nType)
{
	...
	if (m_pCharType == NULL)
	{
		...
		{ 
			if (m_nNumPlayer == 6)
			{
				m_pCharType->SetColoer(D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f));
			}
			else
			{
...
void CResultCharUI::LoadMultRendering(int nCuntry, int nType)
{
	...
	if(m_pCharMultRender != NULL)
	{
		...
		m_pCharMultRender->SetMotionType(CCharMultRender::MOTION_TYPE_LOSE);
	}
}
...
// UIの破棄
//=============================================================================
void CResultCharUI::UninitUI(void)
{
	if (m_pBack != NULL)
	{ //背景
		m_pBack->Uninit();
