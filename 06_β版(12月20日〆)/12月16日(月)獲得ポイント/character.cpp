...
#define COLOR_BULE	 (D3DXCOLOR(0.0f, 0.2f, 1.0f, 1.0f))	//青 安
...
void CCharacter::CountDownLineTime(void)
{
	if (m_apTerritory[TERRITORY_TOP] != NULL)
	{
		...
		if (m_nLineTime < 0)
		{
			...
			{
				CPoint::Create(m_apTerritory[TERRITORY_TOP]->GetPoint(),
					m_apTerritory[TERRITORY_TOP]->GetPos(),
					m_CountryColor[m_nNumPlayerNo]);
			}
...
void CCharacter::AddTerritoryList(CTerritory * pTerritory)
{
	...
	if (m_nCountTerritory >= MIN_TERRITORY)
	{
		...
		{
			int nGetPoint = 0;																					//総合ポイント　安
			ShapeComplete(pTerritory, &nGetPoint);																//図形が完成したときの処理 安
			ChackInShape(&nGetPoint);																			//図形内にテリトリーあるか判定 安
