...
void CRobot::Shoot(void)
{
	...
	else if (m_nNumShoot >= ROBOT_SHOOT_MAXBULLET)
	{
		m_RobotState = ROBOTSTATE_NONE;		// ニュートラル状態に
		m_pMotion->SetNumMotion(m_RobotState);
		m_nCntShoot		= 0;
...
void CRobot::CreateColliderSphere(void)
{
	...
	// スフィアを生成
	CPlayerAttackSphereCollider *pSphere = CPlayerAttackSphereCollider::Create(m_pos,
		D3DXVECTOR3(1.0f, 1.0f, 1.0f), 200.0f, 50, 1);

	//エフェクト
	CEffectManager *pEffectManager = pGame->GetEffectManager();
	pEffectManager->SetEffect(m_pos, INITIALIZE_VECTOR3, 0);
