...
#define ROBOTEVENT_TIMEDEBUG				(300)		// 出現時間(デバック用)
...
CTimer * CGame::m_pTime[2] = {};	// ←追加(よしろう)
...
void CGame::Init(void)
{
	...
	CBullet::LoadModel();				// ロボットの弾	←追加(よしろう)
	...
	//カウントダウンタイマーの生成 ←追加(よしろう)
	if (m_pTime[0] == NULL)
	{
		m_pTime[0] = CTimer::Create(CTimer::TYPE_COUNTER, 7, D3DXVECTOR3(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, 0.0f), D3DXVECTOR3(50.0f, 80.0f, 0.0f));
	}

	//ゲームタイマーの生成（ラスト5秒で描画される） ←追加(よしろう)
	if (m_pTime[1] == NULL)
	{
		m_pTime[1] = CTimer::Create(CTimer::TYPE_GAME, 7, D3DXVECTOR3(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, 0.0f), D3DXVECTOR3(50.0f, 80.0f, 0.0f));
	}
