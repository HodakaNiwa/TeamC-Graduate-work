...
#define ADDHEIGHT_ADDVALUE	(5.5f)
#define ADDDEPTH_ADDVALUE	(-1.7f)
...
bool CGameCamera::CheckTerritoryTopInScreen(CTerritory *pTerritoryTop, D3DXVECTOR3 &pos)
{
	...
	// 画面外判定
	if (pos.x > SCREEN_ADDSIZE_X && pos.x < m_Viewport.Width - SCREEN_ADDSIZE_X
		&& pos.y > SCREEN_ADDSIZE_Y && pos.y < m_Viewport.Height - SCREEN_ADDSIZE_Y)
	{// 画面内に収まっているなら真
		if (pos.x > fValue1 - SCREEN_MIDDLESIZE_X && pos.x < fValue1 + SCREEN_MIDDLESIZE_X
			&& pos.y > fValue2 - SCREEN_MIDDLESIZE_Y && pos.y < fValue2 + SCREEN_MIDDLESIZE_Y)
