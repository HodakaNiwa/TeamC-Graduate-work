...
// 画面エフェクト用
#define UI_SCREENEFFECT_ALPHA      (0.4f)								// 画面効果の透明度
...
void CUI::DrawIcon(int nPlayerIdx)
{
	...
	for (int nCntIcon = 0; nCntIcon < UI_RANKICON_NUM; nCntIcon++)
	{
		if (m_pChraicon[nPlayerIdx][nCntIcon] != NULL)
		{
			...
			CDebugProc::Print("%.1f %.1f %.1f\n", m_pChraicon[nPlayerIdx][nCntIcon]->GetPos().x, m_pChraicon[nPlayerIdx][nCntIcon]->GetPos().y, m_pChraicon[nPlayerIdx][nCntIcon]->GetPos().z);
		}
	}
	CDebugProc::Print("\n");
