...
public:
	...
	static int GetControlerNum(int nNumPlayer) { return m_anControler[nNumPlayer]; }
	static INPUTTYPE GetInputType(int nNumPlayer) { return m_anInputType[nNumPlayer]; }
	LPDIRECT3DTEXTURE9 GetCuntryTex(int nTexNum) { return m_apTextureCountry[nTexNum]; }
	void SetState(STATE state) { m_state = state; }
	STATE GetState(void) { return m_state; }
	CRotationCamera * GetCamera(void) { return m_pCamera; }
...
private:
	...
	void PlayerEnter(int nCntPlayer, INPUTTYPE InputType);
	void SetPlayerInfo(void);
	void UpdateBackCharSelect(void);
	...
	static int m_anControler[NUMPLAYER];				//コントローラー番号の保存
	static INPUTTYPE m_anInputType[NUMPLAYER];			//入力デバイスのタイプ
	...
	bool m_bEntryKeyboard;						//何番目のコントローラーがエントリーしたのか
	bool m_bFadeInGame;
