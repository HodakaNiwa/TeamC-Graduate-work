...
#define TEX_CHARSTATUS	("data/TEXTURE/Select/CharStatus.png")
...
LPDIRECT3DTEXTURE9 CSelectCharacter::m_pStatusTex = NULL;	//安
...
HRESULT CSelectCharacter::Init(D3DXVECTOR3 pos, D3DXVECTOR3 size, int nCuntry, int nNumPlayer, int nNumControler)
{
	...
	if (m_pStatus == NULL)
	{
		m_pStatus = CMoveUI::Create(D3DXVECTOR3(170.0f + (nNumPlayer * 315.0f), 628.0f, 0.0f), D3DXVECTOR3(80.0f, 8.0f, 0.0f), m_pStatusTex);
...
void CSelectCharacter::UpdateInput(void)
{
	...
	//サウンドの取得
	CSound * pSound = CManager::GetSound();	//安
	...
	if (m_InputType == CSelect::INPUTTYPE_GAMEPAD)
	{
		...
		{//右移動
			pSound->PlaySound(CSound::SOUND_LABEL_SE009);	 //セレクト音	安
			MoveRight();
		}
		...
		//決定処理
		if ((pXPad->GetTrigger(XINPUT_GAMEPAD_B, m_nNumControler) == true) || (pKeyboard->GetKeyboardTrigger(DIK_SPACE)) == true)
		{
			pSound->PlaySoundA(CSound::SOUND_LABEL_SE007);	//決定音	安
			PressEnter();
		}
	}
	else
	{
		if (pRawMouse->GetTrigger(m_nNumControler, CRawMouse::RIMS_SLIDE_RIGHT) == true || (pKeyboard->GetKeyboardTrigger(DIK_RIGHT) == true))
		{//右移動
			pSound->PlaySound(CSound::SOUND_LABEL_SE009);	 //セレクト音	安
			MoveRight();
		}
		else if (pRawMouse->GetTrigger(m_nNumControler, CRawMouse::RIMS_SLIDE_LEFT) == true || (pKeyboard->GetKeyboardTrigger(DIK_LEFT) == true))
		{//左移動
			pSound->PlaySound(CSound::SOUND_LABEL_SE009);	 //セレクト音	安
			MoveLeft();
		}
		...
		//決定処理
		if ((pRawMouse->GetTrigger(m_nNumControler, CRawMouse::RIMS_BUTTON_RIGHT) == true) || (pKeyboard->GetKeyboardTrigger(DIK_SPACE)) == true)
		{
			pSound->PlaySoundA(CSound::SOUND_LABEL_SE007);	//決定音	安
			PressEnter();
		}
...
void CSelectCharacter::UpdateScaleUp(D3DXVECTOR3 AimSize, D3DXVECTOR2 Move)
{
	...
	if(m_pStatus != NULL){ m_pStatus->UpdateScaleUp(D3DXVECTOR3(145.0f, 65.0f, 0.0f), Move); } //安
}
...
void CSelectCharacter::UpdateScaleDown(D3DXVECTOR3 AimSize, D3DXVECTOR2 Move)
{
	...
	if (m_pStatus != NULL) { m_pStatus->UpdateScaleDown(D3DXVECTOR3(10.0f, 10.0f, 0.0f), Move); }
}
...
void CSelectCharacter::LoadTex(void)
{
	...
	D3DXCreateTextureFromFile(pDevice, TEX_CHARSTATUS, &m_pStatusTex);	//キャラステータス	安
}
