...
void CResult::Init(void)
{
	...
	int nCharType[MAX_CHARACTER] = {};			//キャラタイプの変数
	int nCuntryType[MAX_CHARACTER] = {};		//国の変数
	int nNumPlayer = CSelect::GetEntryPlayer();	//プレイヤーの総数を取得
	int nNumEnemy = pGame->GetEnemyNumResult();	//エネミーの総数を取得
	
	//プレイヤーとエネミーの総数分まわす
	for (int nCnt = 0; nCnt < nNumPlayer + nNumEnemy; nCnt++)
	{
		//キャラクターの取得
		CCharacter *pCharacter = pGame->GetChara(nCnt);

		//キャラタイプの取得
		nCharType[nCnt] = pCharacter->GetCharcterTypeResult(nCnt);

		//キャラの国を取得
		nCuntryType[nCnt] = pCharacter->GetCuntryResult(nCnt);
	}
	//*******************************************************************
