...
void CGame::Init(void)
{
	...
	{
		m_pCharacter[nCntEnemy] = CTypeSpeed::Create(nCntEnemy, CEnemy::TYPE_BRAZIL, CCharacter::CHARCTERTYPE_SPEED,
			D3DXVECTOR3(0.0f, 0.0f, 0.0f), "data\\TEXT\\ModelLoad\\SPEEDPLAYER_LOAD.txt", "data\\TEXT\\MotionLoad\\SPEEDPLAYER_MOTION.txt");
		m_nEnemyNum++;
		m_nEnemyNumResult++;
	}
	//	★	★	★	★	★	★	★	★	★	★	★	★	★	★	★	★
	...
	{//プレイ人数分回す
		...
		{//スピード型プレイヤーの生成
			m_pCharacter[nCnt] = CSpeedPlayer::Create(D3DXVECTOR3(-50.0f + (nCnt * 50.0f), 0.0f, 0.0f),
				...
				"data\\TEXT\\MotionLoad\\SPEEDPLAYER_MOTION.txt", nCnt, m_nCountry[nCnt], CCharacter::CHARCTERTYPE_SPEED,0,0);	//体下
		}
		...
		{//パワー型プレイヤーの生成
			m_pCharacter[nCnt] = CPowerPlayer::Create(D3DXVECTOR3(-50.0f + (nCnt * 50.0f), 0.0f, 0.0f),
				...
				"data\\TEXT\\MotionLoad\\POWERPLAYER_MOTION.txt", nCnt, m_nCountry[nCnt], CCharacter::CHARCTERTYPE_POWER, 0, 0);
		}
		else if (m_nPlayerType[nCnt] == 2)
		{//地雷型プレイヤーの生成
			m_pCharacter[nCnt] = CTechniquePlayer::Create(D3DXVECTOR3(-50.0f, 0.0f, 0.0f),
				"data\\TEXT\\ModelLoad\\TEQPLAYER_LOAD.txt",
				"data\\TEXT\\MotionLoad\\TEQPLAYER_MOTION.txt", nCnt, CPlayer::TYPE_AMERICA, CCharacter::CHARCTERTYPE_TECHNIQUE,0,0);
		}
	}
}
...
void CGame::CreateUI(void)
{
	...
	for (int nCnt = 0; nCnt < m_nNumPlay; nCnt++)
	{
		m_pUI->CreateSkilicon(m_pCharacter[nCnt], nCnt, 10 * 60);
	}
}
