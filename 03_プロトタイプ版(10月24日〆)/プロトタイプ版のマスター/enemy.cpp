...
void  CEnemy::Move(void)
{
	...
	{
		m_Info[nCnt].fRadian = (float)atan2(m_objPos[nCnt].z - m_beforePos.z, m_objPos[nCnt].x - m_beforePos.x);

	}
	//	次の座標の予測値を計算する
	D3DXVECTOR3 nextPos;
	nextPos.x = (float)cos(m_Info[m_nNum].fRadian) * SPEED + m_beforePos.x;
	nextPos.z = (float)sin(m_Info[m_nNum].fRadian) * SPEED + m_beforePos.z;
...
float CEnemy::dist(D3DXVECTOR3 obj, D3DXVECTOR3 enemy)
{
	...
	//	2点間の距離を計算する
	fOut = (float)sqrt((fInX*fInX) + (fInZ*fInZ));
