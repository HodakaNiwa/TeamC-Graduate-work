...
void CResult::InitWinerChar(int nCuntry, int nType)
{
	...
	case CCharacter::TYPE_JAPAN:
		if (nType == 0)
		{
			m_pWinChar = CCharResult::Create(INITIALIZE_VECTOR3, "data\\TEXT\\ModelLoad\\SPEEDPLAYER_LOAD.txt", "data\\TEXT\\MotionLoad\\SPEEDPLAYER_MOTION.txt",CCharacter::CHARCTERTYPE_SPEED,0);
		}
		else if (nType == 1)
		{
			m_pWinChar = CCharResult::Create(INITIALIZE_VECTOR3, "data\\TEXT\\ModelLoad\\POWERPLAYER_LOAD.txt", "data\\TEXT\\MotionLoad\\POWERPLAYER_MOTION.txt", CCharacter::CHARCTERTYPE_POWER, 0);
		}
		else
		{
			m_pWinChar = CCharResult::Create(INITIALIZE_VECTOR3, "data\\TEXT\\ModelLoad\\TEQPLAYER_LOAD.txt", "data\\TEXT\\MotionLoad\\TEQPLAYER_MOTION.txt",CCharacter::CHARCTERTYPE_TECHNIQUE, 0);
		}
		break;
