	...
	{	
		...
		if (m_pCharBack[nCnt] == NULL)
		{
			...
			{//エントリーしている場合
				...
				{//キャラ選択UIの生成
					Pos.y -= 5.0f;
					m_pSelectChar[nCnt] = CSelectCharacter::Create(Pos, D3DXVECTOR3(40.0f, 40.0f, 0.0f), m_anPlayeCuntry[nCnt], nCnt);
				}
			}
			else
			{//エントリーしていない場合
...
void CSelect::UpdateCharacterSelect(void)
{
	...
	//キーボードの入力処理
	if (pKeyboard->GetKeyboardTrigger(DIK_B) == true) { SetState(STATE_BACKCHANGE); }	//前回の画面に戻る
...
// エントリー処理 
//=============================================================================
void CSelect::PlayerEnter(int nCntPlayer)
{
	//エントリー数の加算
	if(false == m_pNumPlayer[nCntPlayer]->GetDraw()){ m_nEntryPlayer++; }

	//エントリーしている状態にする
	m_bPlayerEnter[nCntPlayer] = true;
		
	//プレイヤーアイコン・国名の表示
	m_pNumPlayer[nCntPlayer]->SetDraw(true);
}
