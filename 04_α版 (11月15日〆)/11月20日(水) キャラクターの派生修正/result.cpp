...
void CResult::InitScoreResult(void)
{
	for (int nCnt = 0; nCnt < MAX_CHARACTER; nCnt++)
	{
		...
		{//キャラ画像
			m_pCharImage[nNo] = CMoveUI::Create(D3DXVECTOR3(80.0f + (nNo * 160.0f), -550.0f, 0.0f), D3DXVECTOR3(80.0f, 275.0f, 0.0f), m_pCharaTex[nNo]);
		}
		...
		{//表
			...
			if (nNo < m_nMaxPlayer)
			{//プレイヤーの色にする
				m_pTable[nNo]->SetColoer(CCharacter::m_CountryColor[nNo]);
			}
			else
			{//灰色にする
				m_pTable[nNo]->SetColoer(D3DXCOLOR(0.3f, 0.3f, 0.3f, 1.0f));
			}
		}
		...
		{//名前のフレーム
			...
			if (nNo < m_nMaxPlayer)
			{//プレイヤーの色にする
				m_pNameFlam[nNo]->SetColoer(CCharacter::m_CountryColor[nNo]);
			}
			else
			{//灰色にする
				m_pNameFlam[nNo]->SetColoer(D3DXCOLOR(0.8f, 0.8f, 0.8f, 1.0f));
			}
		}
