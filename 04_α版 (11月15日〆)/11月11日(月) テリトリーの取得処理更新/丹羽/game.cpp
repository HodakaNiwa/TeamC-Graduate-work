...
void CGame::Init(void)
{
	...
	//	---<<小林が追加しました>>★	★	★	★	★	★	★	★	★	★
	for (int nCntEnemy = 0; nCntEnemy < ENEMY_MAX - m_nNumPlay; nCntEnemy++)
	{
		m_pEnemy[nCntEnemy] = CEnemy::Create(D3DXVECTOR3(0.0f, 0.0f, 0.0f), "data\\TEXT\\ModelLoad\\ENEMY_MODEL.txt", "data\\TEXT\\MotionLoad\\ENEMY_MOTION.txt");
	}
	//	★	★	★	★	★	★	★	★	★	★	★	★	★	★	★	★
...
void CGame::CreateUI(void)
{
	...
	for (int nCnt = 0; nCnt < m_nNumPlay; nCnt++)
	{
		pMiniMap->SetCharacter(m_pPlayer[nCnt], nCnt);
	}
	for (int nCnt = 0; nCnt < ENEMY_MAX - m_nNumPlay; nCnt++)
	{
		pMiniMap->SetCharacter(m_pEnemy[nCnt], (nCnt + m_nNumPlay));
	}
...
void CGame::SettingCharaPosition(void)
{
	...
	int nSetPlayerNum = 0;			// プレイヤーを何人配置したか
	int nSetEnemyNum = 0;			// 敵を何人配置したか
...
void CGame::Update(void)
{
	...
	{
		CMine::Create(m_pPlayer[0]->GetPos(), m_pPlayer[0]);
	}
}
...
void CGame::Draw(void)
{
	...
	if (!m_bDebugCamera)
	{
		...
		else if (m_nNumPlay == 3)
		{
			...
			//カメラの設定
			if (m_pRotationCamera != NULL) { m_pRotationCamera->Set(); }
			CManager::GetRenderer()->GetDevice()->Clear(0,
				NULL,
				(D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER | D3DCLEAR_STENCIL),
				D3DCOLOR_RGBA(0, 0, 0, 0), 1.0f, 0);
