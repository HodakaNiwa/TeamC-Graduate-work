...
HRESULT CResultCharUI::Init(D3DXVECTOR3 pos, D3DXVECTOR3 size, int nNumPlayer, int nRank, int nCuntry, int nType)
{
	...
	if (m_pCharNo == NULL)
	{
		...
		//色の設定
		if (m_pCharNo != NULL) { m_pCharNo->SetColoer(CCharacter::m_CountryColor[nCuntry]); }
	}
	...
	if (m_pCharType == NULL)
	{
		...
		{ 
			...
			{
				m_pCharType->SetColoer(CCharacter::m_CountryColor[nCuntry]);
			}
		}
	}
...
void CResultCharUI::LoadMultRendering(int nCuntry, int nType)
{
	...
	case CCharacter::TYPE_JAPAN:	//日本
		if (nType == 0)
		{
			m_pCharMultRender = CCharMultRender::Create(INITIALIZE_VECTOR3, "data\\TEXT\\ModelLoad\\SPEEDPLAYER_LOAD.txt", "data\\TEXT\\MotionLoad\\SPEEDPLAYER_MOTION.txt",CCharacter::CHARCTERTYPE_SPEED, m_nNumPlayer);
		}
		else if (nType == 1)
		{
			m_pCharMultRender = CCharMultRender::Create(INITIALIZE_VECTOR3, "data\\TEXT\\ModelLoad\\POWERPLAYER_LOAD.txt", "data\\TEXT\\MotionLoad\\POWERPLAYER_MOTION.txt",CCharacter::CHARCTERTYPE_POWER, m_nNumPlayer);
		}
		else
		{
			m_pCharMultRender = CCharMultRender::Create(INITIALIZE_VECTOR3, "data\\TEXT\\ModelLoad\\TEQPLAYER_LOAD.txt", "data\\TEXT\\MotionLoad\\TEQPLAYER_MOTION.txt", CCharacter::CHARCTERTYPE_TECHNIQUE, m_nNumPlayer);
		}
		break;
