...
void CPlayerIcon::UpdateInputXpad(void)
{
	...
	//決定ボタンを押したら
	if (pXPad->GetTrigger(XINPUT_GAMEPAD_A, m_nNumInput) == true)
	{
...
void CPlayerIcon::UpdateInputKeyboard(void)
{
	...
	if (pKeyboard->GetKeyboardTrigger(DIK_UP) == true)
	{//上移動
		m_nOldNumCuntry = m_nNumCuntry;									//前回の情報を保存する
	...
	else if (pKeyboard->GetKeyboardTrigger(DIK_RIGHT) == true)
	{//右移動
		m_nOldNumCuntry = m_nNumCuntry;									//前回の情報を保存する
		m_nNumCuntry += 1;
	...
	else if (pKeyboard->GetKeyboardTrigger(DIK_DOWN) == true)
	{//下移動
		m_nOldNumCuntry = m_nNumCuntry;									//前回の情報を保存する
	...
	else if (pKeyboard->GetKeyboardTrigger(DIK_LEFT) == true)
	{//左移動
		m_nOldNumCuntry = m_nNumCuntry;									//前回の情報を保存する
		m_nNumCuntry -= 1;
...
void CPlayerIcon::UpdateInputMouse(void)
{
	...
	//マウスの角度を取得
	int MouseX = pRawMouse->GetMouse(m_nNumInput).lLastX;
	int MouseY = pRawMouse->GetMouse(m_nNumInput).lLastY;

	if (MouseY < -MOUSE_MOVE)
	{//上移動
	 //セレクト音
		pSound->PlaySound(CSound::SOUND_LABEL_SE009);
	...
	//決定ボタンを押したら
	if (pRawMouse->GetTrigger(m_nNumInput, CRawMouse::RIMS_BUTTON_LEFT) == true)
	{
		if (CFade::FADE_OUT != CFade::GetFadeMode())
		{//フェードアウトの時じゃなかったら音を鳴らす
		 //セレクト音
