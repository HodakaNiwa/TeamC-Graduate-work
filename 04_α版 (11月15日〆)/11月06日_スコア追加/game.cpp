...
void CGame::Init(void)
{
	...
	//m_pEnemy[1] = CEnemy::Create(D3DXVECTOR3(100.0f, 0.0f, 0.0f), "data\\TEXT\\ModelLoad\\ENEMY_MODEL.txt", "data\\TEXT\\MotionLoad\\ENEMY_MOTION.txt");
	//m_pEnemy[2] = CEnemy::Create(D3DXVECTOR3(-100.0f, 0.0f, 0.0f), "data\\TEXT\\ModelLoad\\ENEMY_MODEL.txt", "data\\TEXT\\MotionLoad\\ENEMY_MOTION.txt");
	//m_pEnemy[3] = CEnemy::Create(D3DXVECTOR3(200.0f, 0.0f, 0.0f), "data\\TEXT\\ModelLoad\\ENEMY_MODEL.txt", "data\\TEXT\\MotionLoad\\ENEMY_MOTION.txt");
	//	★	★	★	★	★	★	★	★	★	★	★	★	★	★	★	★
...
void CGame::CreateInitPlayer(void)
{
	...
	{//デバッグ用
		...
		//	if (m_nPlayerType[nCnt] == pSelect->PLAYERTYPE_SPEED1)
			{//スピード型プレイヤーの生成
				m_pPlayer[nCnt] = CSpeedPlayer::Create(D3DXVECTOR3(-50.0f + (nCnt * 50.0f), 0.0f, 0.0f),
	...
	{//デバッグ用
		...
		{//プレイ人数分回す
			...
			//m_nCountry[nCnt] = pSelect->GetCountry(nCnt);		//国を取得
...
void CGame::CreateInitCamera(void)
{
	...
	{//プレイ人数が1人だったら
		...
		{//カメラの生成(1P:左上)
			...
			//if (m_nPlayerType[0] == 1)
			{//スピード型
				m_pGameCamera[0]->Init((CSpeedPlayer*)m_pPlayer[0]);
			}
			//else if (m_nPlayerType[0] == 2)
			//{//パワー型
			//	m_pGameCamera[0]->Init((CPowerPlayer*)m_pPlayer[0]);
			//}
			//else if (m_nPlayerType[0] == 3)
			//{//地雷型
	...
	{//プレイ人数が2人だったら
		...
		{//カメラの生成(1P:左上)
			...
			//if (m_nPlayerType[0] == 2)
			//{//パワー型
			//	m_pGameCamera[0]->Init((CPowerPlayer*)m_pPlayer[0]);
			//}
			//if (m_nPlayerType[0] == 3)
			//{//地雷型
			//	m_pGameCamera[0]->Init(m_pPlayer[0]);
			//}
			m_pGameCamera[0]->SetViewport(viewport);
		...
		{//カメラの生成(2P:右上)
			...
			if (m_nPlayerType[1] == 1)
			{//スピード型
				m_pGameCamera[1]->Init((CSpeedPlayer*)m_pPlayer[1]);
			}
			if (m_nPlayerType[1] == 2)
			{//パワー型型
				m_pGameCamera[1]->Init((CPowerPlayer*)m_pPlayer[1]);
			}
			if (m_nPlayerType[1] == 3)
			{//地雷型
	...
	{//プレイ人数が3人だったら
		...
		{//カメラの生成(3P:左下)
			...
			if (m_nPlayerType[2] == 1)
			{//スピード型
				m_pGameCamera[2]->Init((CSpeedPlayer*)m_pPlayer[2]);
			}
			if (m_nPlayerType[2] == 2)
			{//パワー型
				m_pGameCamera[2]->Init((CPowerPlayer*)m_pPlayer[2]);
			}
			if (m_nPlayerType[2] == 3)
			{//地雷型
	...
	{//プレイ人数が4人だったら
		...
		{//カメラの生成(4P:右下)
			...
			//if (m_nPlayerType[3] == 1)
			{//スピード型
				m_pGameCamera[3]->Init((CSpeedPlayer*)m_pPlayer[3]);
			}
			//if (m_nPlayerType[3] == 2)
			//{//パワー型
			//	m_pGameCamera[3]->Init((CPowerPlayer*)m_pPlayer[3]);
			//}
			//if (m_nPlayerType[3] == 3)
			//{//地雷型
