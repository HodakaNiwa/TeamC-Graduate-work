...
// 生成処理
//=============================================================================
CSelectCharacter * CSelectCharacter::Create(D3DXVECTOR3 pos, D3DXVECTOR3 size, int nCuntry, int nNumPlayer)
{
	CSelectCharacter *pSelectChar = new CSelectCharacter;
	pSelectChar->Init(pos, size, nCuntry, nNumPlayer);
...
// 初期化処理
//=============================================================================
HRESULT CSelectCharacter::Init(D3DXVECTOR3 pos, D3DXVECTOR3 size, int nCuntry, int nNumPlayer)
{
	//変数の初期化
	m_pMoveUI = NULL;
	m_pTypeName = NULL;
...
void CSelectCharacter::UpdateInput(void)
{
	...
	if ((pXPad->GetTrigger(CInputXPad::XPADOTHER_STICK_L_RIGHT, m_nNumPlayer) == true) || (pKeyboard->GetKeyboardTrigger(DIK_RIGHT) == true))
	{//右移動
	...
	else if ((pXPad->GetTrigger(CInputXPad::XPADOTHER_STICK_L_LEFT, m_nNumPlayer) == true) || (pKeyboard->GetKeyboardTrigger(DIK_LEFT) == true))
	{//左移動
		int nState = m_state - 1;
	...
	if ((pXPad->GetTrigger(XINPUT_GAMEPAD_A, m_nNumPlayer) == true) || (pKeyboard->GetKeyboardTrigger(DIK_SPACE)) == true)
	{
...
void CSelectCharacter::LoadMultRendering(void)
{
	...
	case CPlayer::TYPE_JAPAN:	//日本
		...
		m_pCharMultRender[2] = CCharMultRender::Create(D3DXVECTOR3(0.0f, 0.0f, 0.0f), "data\\TEXT\\ModelLoad\\PLAYER_LOAD.txt", "data\\TEXT\\MotionLoad\\PLAYER_MOTION.txt");
		break;
