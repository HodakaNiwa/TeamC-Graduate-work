...
void CResult::Init(void)
{
	...
	m_Color[0] = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);		//１P
	m_Color[1] = D3DXCOLOR(0.25f, 0.5f, 1.0f, 1.0f);	//２P
	m_Color[2] = D3DXCOLOR(1.0f, 0.25f, 0.25f, 1.0f);	//３P
...
void CResult::UpdateScoreResult(void)
{
	...
	if (m_nTime > 140)
	{
		...
		for (int nCnt = 0; nCnt < MAX_CHARACTER; nCnt++)
		{
			m_pGetScore[nCnt] = CScore::Create(CScore::TYPE_GAME, 6, D3DXVECTOR3(40.0f + (160.0f * nCnt), 510.0f, 0.0f), D3DXVECTOR3(20.0f, 30.0f, 0.0f));
		}
	}
	if (m_nTime > 160)
	{
		...
		for (int nCnt = 0; nCnt < MAX_CHARACTER; nCnt++)
		{
			m_pTotalScore[nCnt] = CScore::Create(CScore::TYPE_GAME, 6, D3DXVECTOR3(40.0f + (160.0f * nCnt), 582.0f, 0.0f), D3DXVECTOR3(20.0f, 30.0f, 0.0f));
		}
	}
	if (m_nTime > 180)
	{
		...
		for (int nCnt = 0; nCnt < MAX_CHARACTER; nCnt++)
		{
			m_pRobotedScore[nCnt] = CScore::Create(CScore::TYPE_GAME, 6, D3DXVECTOR3(40.0f + (160.0f * nCnt), 652.0f, 0.0f), D3DXVECTOR3(20.0f, 30.0f, 0.0f));
		}
	}
