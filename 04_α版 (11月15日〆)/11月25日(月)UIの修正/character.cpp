...
void CCharacter::Update()
{
	...
	// 光の軌跡を表示する
	CreateLightOrbit(pGame, m_nNumPlayerNo);
...
void CCharacter::ShapeComplete(CTerritory * pTerritory)
{
	...
	{
		if (m_apTerritory[nCnt] != NULL) { GetTerritory(m_apTerritory[nCnt], nCnt); }
	}
}
		...
		if (CScene::OBJTYPE_TERRITORY == pScene->GetObjType())
		{
			...
			if (pTerritory->GetPlayerNumber() != m_nNumPlayerNo)
			{
				...
				for (int nCntTraiangle = 0; nCntTraiangle < m_nCountTerritory - 2; nCntTraiangle++)
				{
					...
					{
						GetTerritory(pTerritory, nNumLightOrbit);
						nNumLightOrbit++;
						bEnter = true;
						break;
					}
...
// テリトリーの取得処理
//=============================================================================
void CCharacter::GetTerritory(CTerritory * pTerritory, int nNumLightOrbit)
{
	...
	if (nOldNumPlayer != -1)
	{
		CutTerritoryPoint(pTerritory, nOldNumPlayer, nNumLightOrbit);
	}
	//領地獲得音
...
// テリトリーの得点を削除する処理
//=============================================================================
void CCharacter::CutTerritoryPoint(CTerritory * pTerritory, int nOldPlayer, int nNumLightOrbit)
{
	// 得点を取得
...
// 光の軌跡を作る処理
//=============================================================================
void CCharacter::CreateLightOrbit(CGame *pGame, const int nOldPlayer)
{
	// キャラクタークラスを取得
	CCharacter *pChara = pGame->GetChara(nOldPlayer);
	if (pChara == NULL) { return; }
	...
	// 光の軌跡を生成
	CLightOrbit::Create(Pos, &m_Pos, m_nNumPlayerNo);
}
