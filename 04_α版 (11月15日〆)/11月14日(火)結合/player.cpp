...
void  CPlayer::Update(void)
{
	...
	{//ゲーム終了時にモーションをニュートラル
		if (m_bPlayerMotionState == false)
		{
			m_pMotion->SetNumMotion(0);
			m_bPlayerMotionState = true;
		}
	}
	//プレイヤーの状態
...
void  CPlayer::PlayerMove(void)
{
	...
	float fRot = pXPad->GetStickRot(0, nControllerIdx);				//スティックの取得
	if (pXPad->GetStick(0, nControllerIdx) == true)
	{
		//移動状態
		if (m_PlayerState != PLAYERSTATE_BLOWAWAY)
		{
			...
			m_move.x -= sinf(fRot + rot.y) * (fSpeed * m_fSpeed);
			m_move.z -= cosf(fRot + rot.y) * (fSpeed * m_fSpeed);
	...
	{//マウス
		...
		if (m_PlayerState != PLAYERSTATE_BLOWAWAY)
		{
			...
			m_move.x -= sinf((Mouse + rot.y)) * (fSpeed * m_fSpeed);
			m_move.z -= cosf(Mouse + rot.y) * (fSpeed * m_fSpeed);
